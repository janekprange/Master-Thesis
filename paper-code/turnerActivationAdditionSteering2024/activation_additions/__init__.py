...
def get_blocks(model: nn.Module) -> nn.ModuleList:
    ...
    assert (
        len(canidates) == 1
    ), f"Found {len(canidates)} ModuleLists with >50% of model params."
    return canidates[0]


@contextmanager
def residual_stream(
    model: nn.Module, layers: Optional[List[int]] = None
) -> List[t.Tensor]:
    """
    ...
    def _make_hook(i):
        def _hook(_, inputs):
            ...
            stream[i] = (
                inputs[0] if stream[i] is None else t.cat([stream[i], inputs[0]], dim=1)
            )
...
def get_diff_vector(
    model: nn.Module, tokenizer, prompt_add: str, prompt_sub: str, layer: int
):
    """
