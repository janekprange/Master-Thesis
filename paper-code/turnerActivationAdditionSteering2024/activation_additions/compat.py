...
def get_x_vector(
    ...
    act = activation_additions.get_vectors(
        model, model.tokenizer, [prompt1, prompt2], act_name
    )

    return [
        ActivationAddition(
            prompt=prompt1, coeff=coeff, layer=act_name, act=act[0].unsqueeze(0)
        ),
        ActivationAddition(
            prompt=prompt2, coeff=-coeff, layer=act_name, act=act[1].unsqueeze(0)
        ),
    ]
...
class FrequencyPenaltyLogitsProcessor(LogitsProcessor):
    ...
    def __call__(
        self, current_tokens: t.LongTensor, scores: t.FloatTensor
    ) -> t.FloatTensor:
        for batch_index in range(scores.shape[0]):
            scores[batch_index] = scores[
                batch_index
            ] - self.frequency_penalty * t.bincount(
...
def port_sampling_kwargs(sampling_kwargs: Dict[str, float]) -> Dict[str, float]:
    ...
    if "freq_penalty" in sampling_kwargs:
        logit_processors.append(
            FrequencyPenaltyLogitsProcessor(sampling_kwargs["freq_penalty"])
        )
...
def get_n_comparisons(
    ...
    **sampling_kwargs,
    ...
    def _to_df(tokens: t.Tensor, modified: bool):
        completions = [
            model.tokenizer.decode(t.tolist(), skip_special_tokens=True) for t in tokens
        ]
    ...
    inputs, device = (
        model.tokenizer(prompts, return_tensors="pt", padding=True),
        activation_additions._device(model),
    )
    ...
    hooks = [
        (blocks[a.layer], activation_additions.get_hook_fn(a.coeff * a.act))
        for a in additions
    ]
    ...
    nom_df, mod_df = (
        _to_df(nom_tokens, modified=False),
        _to_df(mod_tokens, modified=True),
    )
    ...
    **kwargs,
    ...
    if addition_location != "front":
        raise NotImplementedError(
            "addition_location != front not implemented yet"
        )  # trivial to add, but not needed yet
    ...
    results = get_n_comparisons(
        prompts=prompt_batch, model=model, additions=activation_additions, **kwargs
    )
