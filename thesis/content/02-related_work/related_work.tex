...
% \begin{itemize}
%   \item This figure displays a simplified visualization for a word embedding
%   \item in embeddings, the position of words in the embedding space is not important and has no real meaning, just the relation between points
%   \item points that are close to each other are more similar
%   \item directions correspond to real-world concepts
%   \item most embeddings are created using unsupervised learning methods
%   \item because of that, the dimensions of the embeddings do not directly correspond to real-world concepts as is the case in this illustration
%   \item the characteristic that embeddings that are close to each other are similar to each other and some directions correspond to specific concepts remains true
%   \item there exist methods to relate directions in the embedding space with real-world concepts and make it more interpretable; this thesis presents one such method
% \end{itemize}
