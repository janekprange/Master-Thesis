...
Subsequently, the answers are ordered by their heuristic similarity to the attribute sentence. The positive training sample is the most similar text where the sentence was actually used to describe it in the synthetic dataset. The negative trainings sample is the most dissimilar where the sentence was not used to describe it. If a candidate for the positive sample is not in the top \sfamDataTopPercentText{}, the sentence is not used for the \ac{sfam} training dataset. % TODO: reason
...
\begin{itemize}
  \item \ac{sfam} works well, but the creation of one attribute vector requires \num{\styleVectorSize} forward pass, one per dimension
  \item since this takes too long for practical purposes, a second model, the \acf{lisa} model, is trained based upon the work of \citet{patelLearningInterpretableStyle2023}
  ...
  \item sigmoid activation function to only produce values between 0 and 1
        ...
        \end{itemize}
  \item trained on attribute vectors created by \ac{sfam}
  \item dataset sizes?
\end{itemize}
...
\begin{itemize}
  \item attribute vector vs. attribute embedding
  \item necessary to compare different texts with each other to for example predict from which group a text was written
        % TODO: is the model structure correct?
  \item an embedding head for the \ac{lisa} model with two fully connected layers and a ReLu in between
  \item input: attribute vector; output: attribute embedding
  \item training, validation and test data are the same as \ac{lisa}
  \item trained using triplet loss with a contrastive objective
\end{itemize}
