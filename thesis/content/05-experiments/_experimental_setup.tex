...
Afterward the activation vectors are combined weighted by their importance according the interpretable attribute vector. The result steering vector is added at the layer of the model that it was extracted from during the explanation generation. It is scaled with a factor \(\lambda\) that determines the strength of the steering effect.
